...
class Returns(models.Model):
    ...
    branchid = models.ForeignKey(Branches, on_delete=models.PROTECT)
    date = models.DateTimeField(auto_now_add=True)
    status = models.CharField(max_length=11, choices=STATUS_CHOICES, default='Unprocessed')


    def process_return(returns_data):
        created_returns = []
        ...
        with transaction.atomic():
            for return_data in returns_data:
                ...
                productid = return_data.get("productid")
                quantity = return_data.get("quantity") 
                reason = return_data.get("reason") 
                action = return_data.get("action")
                userid = return_data.get("userid")
                branchid = return_data.get("branchid") 
                
                # Query all Sales instances matching the orderid
                sale_instances = Sales.objects.filter(orderid=order_id)
                ...
                sale_instance = sale_instances.filter(productid=productid).first()
                ...
                # Check for existing returns for this sale and product
                existing_returns = Returns.objects.filter(orderid=order_id, productid=productid, action='Refund')

                # Sum the quantities of existing returns
                total_returned_quantity = sum([ret.quantity for ret in existing_returns])

                # Check if all products have already been returned
                if total_returned_quantity >= sale_instance.quantity:
                    return 'All the products for this sale have already been returned'

                # Check if the return quantity exceeds the remaining quantity
                if total_returned_quantity + quantity > sale_instance.quantity:
                    return 'Return quantity exceeds the remaining quantity available for return'

                # Process inventory if action is 'Replace'
                if action == 'Replace':
                    inventory_items = Inventory.objects.filter(productid=productid)[:quantity]

                    if inventory_items.count() < quantity:
                ...
                created_returns.append(return_instance) 
